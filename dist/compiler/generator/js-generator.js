import { PRECEDENCE as BIN_PRECEDENCE } from "../../language/precedence.js";
import { rewriteImportSource } from "../resolver.js";
import { SourceMapBuilder } from "./source-map.js";
export function generateJS(program, minify) {
  const min = minify ?? false;
  const nl = min ? "" : "\n";
  const sp = min ? "" : " ";
  const ctx = { minify: min, nl: nl, sp: sp, indentLevel: 0, indentSize: 2, declaredVars: new Set() };
  const lines = program.body.map(stmt => emitStatement(stmt, ctx));
  return lines.join(nl);
}
export function generateJSWithSourceMap(program, sourceFile, sourceContent, outputFile, minify) {
  ...
  return { js: js, sourceMap: sourceMap };
}
  ...
  switch (expr.type) {
    ...
    case "ComptimeExpression": {
      ...
      if (expr.body) {
        ...
        const evaluator2 = new Function("return (" + code + ")");
    ...
    case "IfExpression": {
      ...
      return "(() =>" + ctx.sp + "{" + ctx.sp + "if" + ctx.sp + "(" + cond + ")" + ctx.sp + "{" + ctx.sp + thenBody + ctx.sp + "}" + ctx.sp + "else" + ctx.sp + "{" + ctx.sp + elseBody + ctx.sp + "}" + ctx.sp + "})()";
    }
    default: {
