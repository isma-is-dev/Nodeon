...
| Tests | ✅ Good | 627 tests (lexer 35, parser 84, e2e 211, bootstrap 98, type-checker 81, nova-signals 21, nova-template 32, nova-di 21, nova-island-bundler 13, regression 26, snapshot 5) |
...
| `e2e.test.ts` | 211 | Full compile pipeline, output verification, named arguments, ADTs, pattern matching v2, go concurrency, comptime, IR pipeline |
...
- [x] ~~**Compile-time evaluation**~~ ✅ `comptime 2 ** 10` → `1024`, `comptime { let x = 10; x * x }` → `100` — expressions/blocks evaluated at compile time via `new Function`
- [ ] **WebAssembly backend** — Compile `.no` to WASM
- [ ] **Standalone binaries** — Bundle to single executable (via Node SEA or similar)
- [x] ~~**IR layer**~~ ✅ `IRModule` → `IRFunction` → `IRBlock` → `IRInstruction[]` with constant folding + dead code elimination optimization passes, `compileWithIR()` API
